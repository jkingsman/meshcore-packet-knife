...
export interface GpuBruteForceCallbacks {
  onProgress: (checked: number, rate: number, current: string) => void;
  onCandidate: (roomName: string, key: string) => boolean; // return true to stop
  shouldAbort: () => boolean;
}
...
// Singleton instance
let gpuBruteForce: GpuBruteForce | null = null;

export async function getGpuBruteForce(): Promise<GpuBruteForce | null> {
  if (gpuBruteForce) {
